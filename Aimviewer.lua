...
strafeConn     = nil
local strafeFrame   = 0
local strafeALTimer = 0
local strafeALNudge = 0
...
function startStrafe(targetPlayer)
    ...
    local rel = myHrp.Position - tHrp.Position
    strafeTheta = math.atan2(rel.Z, rel.X)

    local function calcBPValues()
        local r = math.max(STRAFE.Radius, 1)
        local P = math.clamp(8000 + r * 600, 20000, 500000)
        local D = P * 0.08
        return P, D
    end

    local initP, initD = calcBPValues()

    local bp = Instance.new("BodyPosition")
    bp.Name     = "SF_BodyPos"
    bp.MaxForce = Vector3.new(1e7, 1e7, 1e7)  -- raised to handle large radius
    bp.D        = initD
    bp.P        = initP
    ...
    strafeConn = RunService.RenderStepped:Connect(function(dt)
        ...
        -- Frame skip
        strafeFrame = strafeFrame + 1
        local skip = STRAFE.FrameSkip + 1
        if strafeFrame % skip ~= 0 then return end
        local eff_dt = dt * skip

        -- Anti-lock
        strafeALTimer = strafeALTimer + eff_dt
        if STRAFE.AntiLock and strafeALTimer >= 0.5 then
            strafeALTimer = 0
            strafeALNudge = (math.random() - 0.5) * 0.4
        end
        if not STRAFE.AntiLock then strafeALNudge = 0 end

        local radPerSec = STRAFE.Speed * 0.06
        local step = radPerSec * STRAFE.Direction * eff_dt
        step = math.clamp(step, -math.pi/6, math.pi/6)
        strafeTheta = strafeTheta + step + strafeALNudge

        local tPos = tHrpLive.Position
        local r    = math.max(STRAFE.Radius, 0.1)

        local destX = tPos.X + math.cos(strafeTheta) * r
        local destY = tPos.Y + STRAFE.HeightOffset
        local destZ = tPos.Z + math.sin(strafeTheta) * r

        local newP, newD = calcBPValues()
        bp.P = newP; bp.D = newD

        if STRAFE.Jitter then
            local ja = STRAFE.JitterAmt
            local jx = (math.random()-0.5)*2*ja
            local jy = (math.random()-0.5)*2*ja
            local jz = (math.random()-0.5)*2*ja
            myHrpLive.CFrame = CFrame.new(destX+jx, destY+jy, destZ+jz)
                * CFrame.Angles(
                    math.rad((math.random()-0.5)*30),
                    math.rad((math.random()-0.5)*360),
                    math.rad((math.random()-0.5)*30))
        else
            bp.Position = Vector3.new(destX, destY, destZ)
        end

        if STRAFE.Spin then
            local spinAngle = tick() * STRAFE.SpinSpeed
            bg.CFrame = CFrame.new(myHrpLive.Position) * CFrame.Angles(0, spinAngle, 0)
        else
            local lookDir = tPos - myHrpLive.Position
            if lookDir.Magnitude > 0.1 then
                bg.CFrame = CFrame.new(myHrpLive.Position, tPos)
            end
        end

        -- Optional aimbot snap on top of spectate (snaps to hitbox precisely)
        if STRAFE.LockCamera then
            local tHitbox = getHitbox(STRAFE.TargetPlayer)
            local aimPoint = tHitbox and tHitbox.Position or tPos
...
sfMaster.Size = UDim2.new(1,0,0,0)
sfMaster.AutomaticSize = Enum.AutomaticSize.Y
...
sfML.FillDirection = Enum.FillDirection.Vertical
...
sfSub.BackgroundTransparency = 1; sfSub.Text = "11phhsware  |  strafe engine v3.1"
...
function setSfBadge(active)
    if active then
        ...
        sfBadgeTxt.Text = "STRAFING"; sfPulseTw:Play()
    else
        sfPulseTw:Cancel()
...
sfX.MouseEnter:Connect(function()
    tw(sfX,{BackgroundColor3=Color3.fromRGB(160,30,30),BackgroundTransparency=0,TextColor3=Color3.fromRGB(255,100,100)})
end)
sfX.MouseLeave:Connect(function()
    tw(sfX,{BackgroundColor3=Color3.fromRGB(38,16,16),BackgroundTransparency=0.2,TextColor3=Color3.fromRGB(180,70,70)})
end)

local sfSetPanel = Instance.new("Frame", sfMaster)
sfSetPanel.Size = UDim2.new(1,-16,0,0)
sfSetPanel.AutomaticSize = Enum.AutomaticSize.Y
sfSetPanel.Position = UDim2.new(0,8,0,0)
...
local function mkSfToggle(parent, label, initVal, lo, onChange)
    ...
    lbl.TextColor3 = Color3.fromRGB(180,170,200); lbl.Font = Enum.Font.GothamMedium
...
mkSfSlider(sfSetPanel, "Strafe Radius",   0,  300, STRAFE.Radius,        " st",  1, function(v) STRAFE.Radius = v       end)
mkSfSlider(sfSetPanel, "Strafe Speed",    1, 1000, STRAFE.Speed,          "",     2, function(v) STRAFE.Speed = v        end)
mkSfSlider(sfSetPanel, "Height Offset", -50,  300, STRAFE.HeightOffset,  " st",  3, function(v) STRAFE.HeightOffset = v end)
mkSfDivider(sfSetPanel, 4)

local dirRow = Instance.new("Frame", sfSetPanel)
dirRow.Size = UDim2.new(1,0,0,32); dirRow.BackgroundTransparency = 1; dirRow.LayoutOrder = 5
local dirLbl = Instance.new("TextLabel", dirRow)
dirLbl.Size = UDim2.new(0.6,0,1,0); dirLbl.BackgroundTransparency = 1
dirLbl.Text = "Direction"; dirLbl.TextColor3 = Color3.fromRGB(180,170,200)
dirLbl.Font = Enum.Font.GothamMedium; dirLbl.TextSize = 10
dirLbl.TextXAlignment = Enum.TextXAlignment.Left
local dirBtn = Instance.new("TextButton", dirRow)
dirBtn.Size = UDim2.new(0,82,0,22); dirBtn.Position = UDim2.new(1,-83,0.5,-11)
dirBtn.BackgroundColor3 = Color3.fromRGB(40,15,80); dirBtn.BorderSizePixel = 0
dirBtn.Text = "◁  CW"; dirBtn.TextColor3 = Color3.fromRGB(160,90,255)
dirBtn.Font = Enum.Font.GothamBold; dirBtn.TextSize = 10
Instance.new("UICorner", dirBtn).CornerRadius = UDim.new(0,5)
local dts = Instance.new("UIStroke", dirBtn)
dts.Color = Color3.fromRGB(100,40,180); dts.Thickness = 1
dts.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
dirBtn.MouseButton1Click:Connect(function()
    STRAFE.Direction = -STRAFE.Direction
    dirBtn.Text = STRAFE.Direction == 1 and "◁  CW" or "▷  CCW"
end)
dirBtn.MouseEnter:Connect(function() tw(dirBtn,{BackgroundColor3=Color3.fromRGB(60,22,110)},FAST) end)
dirBtn.MouseLeave:Connect(function() tw(dirBtn,{BackgroundColor3=Color3.fromRGB(40,15,80)},FAST)  end)

mkSfDivider(sfSetPanel, 6)
mkSfToggle(sfSetPanel, "Lock Camera (Aimbot)",  STRAFE.LockCamera, 7,  function(v) STRAFE.LockCamera = v  end)
mkSfToggle(sfSetPanel, "Character Jitter",      STRAFE.Jitter,     8,  function(v) STRAFE.Jitter = v      end)
mkSfSlider(sfSetPanel, "Jitter Amount",   1, 15, STRAFE.JitterAmt,  " st", 9,  function(v) STRAFE.JitterAmt = v  end)
mkSfToggle(sfSetPanel, "Character Spin",        STRAFE.Spin,       10, function(v) STRAFE.Spin = v        end)
mkSfSlider(sfSetPanel, "Spin Speed",      1, 50, STRAFE.SpinSpeed,  " r/s",11, function(v) STRAFE.SpinSpeed = v  end)
mkSfDivider(sfSetPanel, 12)
mkSfSlider(sfSetPanel, "Frame Skip",      0, 10, STRAFE.FrameSkip,  "",   13, function(v) STRAFE.FrameSkip = v  end)
mkSfToggle(sfSetPanel, "Anti-Lock",             STRAFE.AntiLock,   14, function(v) STRAFE.AntiLock = v   end)
...
sfStopRow.Size = UDim2.new(1,-16,0,34); sfStopRow.BackgroundTransparency = 1
...
sfStopBtn.Text = "⬛  STOP STRAFE"; sfStopBtn.TextColor3 = Color3.fromRGB(255,80,80)
...
sfStopBtn.MouseEnter:Connect(function() tw(sfStopBtn,{BackgroundColor3=Color3.fromRGB(130,30,30)},FAST) end)
sfStopBtn.MouseLeave:Connect(function() tw(sfStopBtn,{BackgroundColor3=Color3.fromRGB(80,20,20)},FAST) end)
...
local sfStopPad = Instance.new("UIPadding", sfStopRow)
sfStopPad.PaddingLeft = UDim.new(0,0); sfStopPad.PaddingRight = UDim.new(0,0)

local sfListHdrRow = Instance.new("Frame", sfMaster)
sfListHdrRow.Size = UDim2.new(1,-16,0,22); sfListHdrRow.BackgroundTransparency = 1
...
sfDiv2.Size = UDim2.new(1,-16,0,1); sfDiv2.BackgroundColor3 = Color3.fromRGB(50,25,80)
sfDiv2.BorderSizePixel = 0; sfDiv2.LayoutOrder = 5

local sfScroll = Instance.new("ScrollingFrame", sfMaster)
sfScroll.Size = UDim2.new(1,-16,0,130)
...
function rebuildSfList()
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LP then
            ...
            dot.BackgroundColor3 = isLocked and Color3.fromRGB(180,100,255) or Color3.fromRGB(55,215,125)
            dot.BorderSizePixel = 0
            Instance.new("UICorner", dot).CornerRadius = UDim.new(1,0)
            local nm = Instance.new("TextLabel", row)
            nm.Size = UDim2.new(0,140,1,0); nm.Position = UDim2.new(0,20,0,0)
            nm.BackgroundTransparency = 1; nm.Text = p.Name
            nm.TextColor3 = isLocked and Color3.fromRGB(210,180,255) or Color3.fromRGB(200,200,220)
            ...
            lockBtn.BackgroundColor3 = isLocked and Color3.fromRGB(80,20,140) or Color3.fromRGB(40,15,80)
            lockBtn.Text = isLocked and "STRAFING" or "LOCK"
            lockBtn.TextColor3 = isLocked and Color3.fromRGB(220,160,255) or Color3.fromRGB(160,90,255)
            ...
            lkStroke.Color = isLocked and Color3.fromRGB(160,60,255) or Color3.fromRGB(100,40,180)
            ...
            table.insert(sfListConns, lockBtn.MouseButton1Click:Connect(function()
                if STRAFE.TargetPlayer == p then
                    stopStrafe(); setSfBadge(false)
                    tw(lockBtn,{BackgroundColor3=Color3.fromRGB(40,15,80),TextColor3=Color3.fromRGB(160,90,255)},MED)
                    lockBtn.Text = "LOCK"
                    ...
                    tw(lockBtn,{BackgroundColor3=Color3.fromRGB(80,20,140),TextColor3=Color3.fromRGB(220,160,255)},MED)
                    tw(lkStroke,{Color=Color3.fromRGB(160,60,255)},MED)
                    lockBtn.Text = "STRAFING"
            ...
            distConn = RunService.Heartbeat:Connect(function()
                ...
                if p.Character and lpRoot then
                    ...
                    if hrpT then
                        ...
                        distLbl.Text = d.."st"
                        distLbl.TextColor3 = d < 15 and Color3.fromRGB(255,180,50) or Color3.fromRGB(70,55,95)
                    end
